...
//url="jdbc:mysql://localhost:3306/prueba? useUnicode=true&use"
    ...
    private void registarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_registarActionPerformed
        ...
        if (textonombre.getText().equals(" ") || textoapellido.getText().equals(" ") || textodireccion.getText().equals(" ")
            ...
            try {
                ...
                CallableStatement insert = (CallableStatement) con.prepareCall("call Insertar_cliente (?,?,?,?,?,?)");
               // ps = (PreparedStatement) con.prepareStatement("insert into cliente(Nombre,Apellido,Direccion,Dni,Telefono,correo) values(?,?,?,?,?,?)");
                insert.setString(1, textonombre.getText());
                insert.setString(2, textoapellido.getText());
                insert.setString(3, textodireccion.getText());
                insert.setString(4, textodni.getText());
                insert.setString(5, textotelefono.getText());
                insert.setString(6, textocorreo.getText());
    ...
    private void textocorreoKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_textocorreoKeyTyped
        if (textocorreo.getText().length() >= 40) {
            evt.consume();
            Toolkit.getDefaultToolkit().beep();
        }
